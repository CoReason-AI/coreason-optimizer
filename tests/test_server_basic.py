...
def test_optimize_endpoint(mock_metric: MagicMock, mock_bootstrap: MagicMock, mock_mipro: MagicMock) -> None:
    # Setup mocks
...
async def test_bridged_client() -> None:
    ...
    def worker() -> str:
        return str(bridge.generate(messages=[]))
...
def test_optimize_semantic_success(mock_metric: MagicMock, mock_mipro: MagicMock) -> None:
    ...
    mock_optimizer_instance.compile.return_value = OptimizedManifest(
        agent_id="test", base_model="gpt", optimized_instruction="sys", performance_metric=1.0, optimization_run_id="id"
    )

    payload = {
        "agent": {"system_prompt": "Original prompt", "inputs": ["input1"], "outputs": ["output1"]},
        "dataset": [{"inputs": {"input1": "val1"}, "reference": "ref1", "metadata": {}}],
        "config": {"metric": "exact_match", "selector_type": "semantic"},
        "strategy": "mipro",
    }
