...
@pytest.mark.asyncio
async def test_random_selector_selection() -> None:
...
@pytest.mark.asyncio
async def test_random_selector_oversized_request() -> None:
    examples = [
...
@pytest.mark.asyncio
async def test_random_selector_determinism() -> None:
    ...
    sel1 = await s1.select(ds, k=5)
    sel2 = await s2.select(ds, k=5)
