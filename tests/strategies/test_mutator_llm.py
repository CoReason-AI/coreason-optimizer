...
def test_llm_mutator_success():
    ...
    # call_args[1] is kwargs (model=..., temperature=...)
    # call_args[0] is positional args (messages=...) IF passed positionally, but client.generate signature is generate(self, messages, ...) or kwargs?
    # Based on core/client.py: generate(self, messages: list[dict], model: str, ...)
...
def test_llm_mutator_cleanup_markdown():
...
def test_llm_mutator_empty_response():
...
def test_llm_mutator_budget_exceeded():
...
def test_llm_mutator_generic_exception():
