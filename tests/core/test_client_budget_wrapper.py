...
@pytest.mark.asyncio
async def test_wrapper_delegates_and_tracks_cost() -> None:
    ...
    await wrapper.generate([{"role": "user", "content": "hi"}])
    assert budget.total_cost_usd == 2.0


@pytest.mark.asyncio
async def test_wrapper_raises_budget_exceeded() -> None:
...
@pytest.mark.asyncio
async def test_wrapper_passes_args() -> None:
    ...
    await wrapper.generate(messages=[{"a": "b"}], model="gpt-test", temperature=0.7, extra="val")

    mock_inner.generate.assert_called_once_with(messages=[{"a": "b"}], model="gpt-test", temperature=0.7, extra="val")


@pytest.mark.asyncio
async def test_wrapper_blocks_call_if_already_exceeded() -> None:
...
@pytest.mark.asyncio
async def test_boundary_conditions() -> None:
